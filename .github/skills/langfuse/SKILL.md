...
> Installed from https://github.com/langfuse/skills into this repo at `.github/skills/langfuse/`. Only the reference files needed for instrumentation (`references/instrumentation.md`, `references/cli.md`) were vendored; the other use-case references listed below can be fetched from the upstream repo on demand if needed.
...
- migrating prompts from a codebase into Langfuse: references/prompt-migration.md (not vendored — fetch from upstream if needed)
- creating a prompt or changing any part of an existing prompt, including small edits and debugging/tuning: references/prompt-engineering.md (not vendored — fetch from upstream if needed)
- capturing user feedback (thumbs, ratings, implicit signals) as scores on traces: references/user-feedback.md (not vendored — fetch from upstream if needed)
- further tips on using the Langfuse CLI: references/cli.md
- upgrading or migrating Langfuse SDKs and preserving application instrumentation attributes: references/sdk-upgrade.md (not vendored — fetch from upstream if needed)
- upgrading legacy trace-level or dataset-item evaluators to observation-level or experiment evaluators: references/trace-evaluator-upgrade.md (not vendored). Use the [evaluator migration guide](https://langfuse.com/faq/all/llm-as-a-judge-migration) as the primary reference.
- preparing a Langfuse project for the v4 platform migration: references/v4-project-migration.md (not vendored — fetch from upstream if needed)
- judge calibration (LLM-as-a-Judge reliability, simple accuracy checks, advanced split-based validation, confusion matrices, and metric ingestion): references/judge-calibration.md (not vendored)
- systematic error analysis — reading traces, building failure taxonomy, deciding what to fix: references/error-analysis.md (not vendored)
- setting up CI/CD experiment gates with `langfuse/experiment-action`: references/ci-cd.md (not vendored)
- submitting feedback about this skill: references/skill-feedback.md (not vendored)
...
Note that changelog posts may also surface here: use them only to confirm a feature exists, never to implement from — their examples may be outdated, so always implement from the docs and API/SDK reference.
...
When triggered, follow the process described in the upstream `references/skill-feedback.md` (not vendored locally — fetch from https://github.com/langfuse/skills if needed), or point the user to https://github.com/langfuse/skills/discussions/new?category=ideas-improvements.
